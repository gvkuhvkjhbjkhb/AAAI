#!/usr/bin/env bash
# Start the P3 servers with the exact revision pins recorded after S2.
set -euo pipefail

P3_ROOT="${P3_ROOT:?Set P3_ROOT to P3_TRANSFER_EXPERIMENT_v1}"
PYTHON_BIN="${PYTHON_BIN:-python3}"
VLLM_LOG_DIR="${VLLM_LOG_DIR:-$P3_ROOT/logs/vllm_p3}"
...
for attempt in $(seq 1 90); do
  ...
  if [[ "$qwen" == *"$QWEN_MODEL"* && "$glm" == *"$GLM_MODEL"* ]]; then
    echo "P3 servers ready: Qwen PID=$PID0; GLM PID=$PID1."
...
echo "Timed out waiting for P3 vLLM servers." >&2

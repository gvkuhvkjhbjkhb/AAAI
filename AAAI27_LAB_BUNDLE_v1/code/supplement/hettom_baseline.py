...
class LLMAgent:
    ...
    def __init__(self, agent_id, model_name, temperature, role_label,
        ...
        # top_p for nucleus sampling (0.9 = frozen main-table config; 1.0 = the
        # G2 single-factor ablation that isolates sampling-truncation effects).
        ...
        self.role_asymmetric_hint = role_asymmetric_hint
        self.history_split_hint = history_split_hint
        self.adaptive_intervention = adaptive_intervention
        self.adaptive_interv_threshold = adaptive_interv_threshold
        self._own_action_history = []
        self._n_conflicts = 0
    ...
    def _build_action_prompt(self, game, obs, teammate_preds=None, signals=None):
        ...
        if self.payoff_in_prompt and game.base.n_agents == 2:
            ...
            self_index = int(self.agent_id) - 1
            if self_index not in (0, 1):
                raise ValueError("Two-player payoff prompt requires agent_id 1 or 2")
            rows = []
            for self_action in range(na):
                cells = []
                for teammate_action in range(na):
                    joint = ((self_action, teammate_action) if self_index == 0
                             else (teammate_action, self_action))
                    rewards = game.base.payoff_vector(joint)
                    self_reward = rewards[self_index]
                    teammate_reward = rewards[1 - self_index]
                    cells.append(f"({self_reward:.1f},{teammate_reward:.1f})")
                rows.append(f"  {names[self_action]:>10}: " + "  ".join(cells))
            payoff_line = (
    ...
    def act(self, game, obs, teammate_ids, signals=None):
        ...
        if self.gated_talk_tom and signals:
            ...
            if self.diversity_preserving_gate:
                ...
                if has_conflict:
                    self._n_conflicts += 1
                    # --- Scheme 1: adaptive intervention ---
                    do_intervene = True
                    if self.adaptive_intervention:
                        total = self._n_conflicts
                        if total > 0:
                            interv_rate = self._n_interventions / total
                            if interv_rate > self.adaptive_interv_threshold:
                                do_intervene = (self._rng.random() >
                                                (interv_rate - self.adaptive_interv_threshold))
                    if do_intervene:
...
def build_agents(config):
    ...
    for i in range(n):
        ...
        agents.append(LLMAgent(
            ...
            role_asymmetric_hint=config.get("role_asymmetric_hint", False),
            history_split_hint=config.get("history_split_hint", False),
            adaptive_intervention=config.get("adaptive_intervention", False),

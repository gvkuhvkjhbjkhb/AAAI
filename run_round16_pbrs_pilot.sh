...
WORKER_BASE="${WORKER:-/data/lab/AAAI_worker_round16_pbrs}"
GPU="${GPU:-0}"
PARALLEL="${PARALLEL:-1}"
...
main(){
  log_status "Round 16 PBRS pilot started at $(date) PARALLEL=$PARALLEL"
  ...
  wait "${pids[@]}"
  package_outputs
  log_status "Round 16 PBRS pilot finished at $(date)"
}
main "$@"

...
## Implemented Fast One-GPU Queue

The executable Round 7 script is `run_round7_targeted_aaai_one_gpu.sh`. It runs the decisive 44-job subset on one RTX 5090: eight seeds for the three matched random-type controls and the new confidence-gated semantic method on 10x10, plus seeds 6-8 for baseline, uniform 0.0002, adaptive 0.0003 late 0.45, and semantic-gate on 12x12. Each completed run appends a `DONE ... elapsed=...` line to `STATUS.txt`, writes a `.done` marker beside the run log, and refreshes `summary.csv`, `summary.txt`, and `ROUND7_LIVE_AGGREGATES.md`. Based on Round 6 timings, expected wall time is about 4.5-6.0 hours on a single 5090.

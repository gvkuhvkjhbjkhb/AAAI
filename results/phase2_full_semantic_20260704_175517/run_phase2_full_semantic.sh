...
from pathlib import Path
out = Path(sys.argv[1])
mapping = {r['source_file'] + '|' + r['source_line']: r['sample_id'] for r in csv.DictReader(open(out / 'human_labels_adjudicated.csv', encoding='utf-8'))}
for sub in ['original_heuristic', 'enhanced_heuristic', 'qwen35_4b']:
    ...
    with open(out / f'{sub}_by_sample.csv', 'w', newline='', encoding='utf-8') as h:
        ...
        w.writeheader()
        for r in rows:
            r['sample_id'] = mapping.get(r['source_file'] + '|' + r['source_line'], '')
            w.writerow(r)
PY
}

...
make_jobs(){
  ...
  for seed in 1 9; do
    for method in baseline fa_pbrs_l002 pbrs_fixed_l002 pbrs_random_features_l002; do
      printf 'long800|lbforaging:Foraging-10x10-3p-3f-v3|50|800000|%s|%s\n' "$method" "$seed" >> "$OUT_DIR/jobs_all.txt"
    done
  done
  for env_key in lbforaging:Foraging-8x8-2p-2f-v3 lbforaging:Foraging-10x10-2p-3f-v3 lbforaging:Foraging-15x15-3p-5f-v3; do
    for seed in 1 9; do
      for method in baseline fa_pbrs_l002 pbrs_random_features_l002; do
        printf 'env300|%s|50|300000|%s|%s\n' "$env_key" "$method" "$seed" >> "$OUT_DIR/jobs_all.txt"
      done
    done
  done
  python3 - "$OUT_DIR/jobs_all.txt" <<'PYSHUFFLE'
import random, sys
p=sys.argv[1]
rows=open(p, encoding='utf-8').readlines(); random.Random(18).shuffle(rows); open(p,'w',encoding='utf-8').writelines(rows)
PYSHUFFLE
}

...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/results/v2_pilot_${STAMP}}"
LOG_DIR="$OUT_DIR/logs"
STATUS="$OUT_DIR/STATUS.txt"
WORKER_BASE="${WORKER:-/data/lab/AAAI_worker_v2}"
GPU="${GPU:-0}"
PARALLEL="${PARALLEL:-5}"
...
T_MAX="${T_MAX:-500000}"
SEEDS="${SEEDS:-1 2 3 4}"
METHODS="${METHODS:-baseline fa_pbrs_l002 dg_pbs_v2 fme_l01}"
...
method_extra(){
  ...
  case "$method" in
    ...
    dg_pbs_v2) extra="llm_fd_enabled=True llm_fd_classifier=enhanced_heuristic llm_fd_max_records=$MAX_RECORDS llm_fd_apply_reward_shaping=True llm_fd_intervention_mode=dg_pbs llm_fd_pbrs_lambda=0.02 llm_fd_pbrs_alpha=0.01 llm_fd_pbrs_beta=0.05 llm_fd_pbrs_adaptive=True llm_fd_dg_window=50 llm_fd_dg_warmup=20 llm_fd_dg_smoothing=0.8 llm_fd_dg_temperature=5.0 llm_fd_dg_gate_max=2.0 llm_fd_dg_gate_min=0.1" ;;
    fme_l01) extra="llm_fd_enabled=True llm_fd_classifier=enhanced_heuristic llm_fd_max_records=$MAX_RECORDS llm_fd_apply_reward_shaping=True llm_fd_intervention_mode=fme llm_fd_pbrs_lambda=0.02 llm_fd_pbrs_alpha=0.01 llm_fd_pbrs_beta=0.05 llm_fd_pbrs_adaptive=True llm_fd_fme_bonus_scale=0.01 llm_fd_fme_window=50 llm_fd_fme_warmup=20 llm_fd_fme_smoothing=0.8 llm_fd_fme_max_scale=3.0 llm_fd_fme_min_scale=0.5" ;;
    *) return 2 ;;
  esac
  printf '%s|%s\n' "$config" "$extra"
}
make_jobs(){
  ...
  for seed in $SEEDS; do for method in $METHODS; do printf 'v2|%s|%s|%s|%s|%s\n' "$ENV_KEY" "$TIME_LIMIT" "$T_MAX" "$method" "$seed" >> "$OUT_DIR/jobs_all.txt"; done; done
  if [[ "${SHUFFLE_JOBS:-1}" == "1" ]]; then python3 - "$OUT_DIR/jobs_all.txt" "${JOB_SHUFFLE_SEED:-22}" <<'PYSHUFFLE'
import random, sys
...
package_outputs(){ summarize_partial; local artifact="$ROOT_DIR/artifacts/AAAI_v2_pilot_${STAMP}.tar.gz"; tar -C "$ROOT_DIR" -czf "$artifact" "results/$(basename "$OUT_DIR")" "run_v2_pilot.sh"; log_status "ARTIFACT $artifact"; }
main(){
  log_status "V2 pilot started at $(date) GPU=$GPU PARALLEL=$PARALLEL methods=$METHODS seeds=$SEEDS t_max=$T_MAX"
  ...
  wait "${pids[@]}"
  package_outputs
  log_status "V2 pilot finished at $(date)"
}
main "$@"

...
WORKER_BASE="${WORKER:-/data/lab/AAAI_worker_round15_redesign}"
GPU="${GPU:-0}"
PARALLEL="${PARALLEL:-1}"
...
main(){
  log_status "Round 15 redesign pilot started at $(date) PARALLEL=$PARALLEL"
  ...
  wait "${pids[@]}"
  package_outputs
  log_status "Round 15 redesign pilot finished at $(date)"
}
main "$@"

...
S1 is intentionally not run yet: it is a preregistered, held-out experiment
that tests whether a label-free controller can align only when warm-up behavior
certifies sufficient coverage. It treats the current G1/G3 failures as the
central online-identification boundary, not as results to hide. See
[`S1_SAFE_SCA_PROTOCOL.md`](S1_SAFE_SCA_PROTOCOL.md) for the exact decision
rule, development/held-out split, commands, output contract, and success
criteria.

...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/results/ab_validation_3seed_${STAMP}}"
LOG_DIR="$OUT_DIR/logs"
STATUS="$OUT_DIR/STATUS.txt"
WORKER_BASE="${WORKER:-/data/lab/AAAI_worker_ab_val}"
GPU="${GPU:-0}"
PARALLEL="${PARALLEL:-3}"
...
T_MAX="${T_MAX:-1000000}"
SEEDS="${SEEDS:-1 2 3}"
METHODS="${METHODS:-baseline fa_pbrs_l002}"
...
method_extra(){
  ...
  case "$method" in
    ...
    pbrs_fixed_l002) extra="llm_fd_enabled=False llm_fd_apply_reward_shaping=True llm_fd_intervention_mode=fa_pbrs llm_fd_pbrs_lambda=0.02 llm_fd_pbrs_alpha=0.0 llm_fd_pbrs_beta=0.0 llm_fd_pbrs_adaptive=False" ;;
    pbrs_random_weights_l002) extra="llm_fd_enabled=False llm_fd_apply_reward_shaping=True llm_fd_intervention_mode=fa_pbrs llm_fd_pbrs_lambda=0.02 llm_fd_pbrs_alpha=0.0 llm_fd_pbrs_beta=0.0 llm_fd_pbrs_adaptive=False llm_fd_pbrs_random_weights=True" ;;
    pbrs_random_features_l002) extra="llm_fd_enabled=False llm_fd_apply_reward_shaping=True llm_fd_intervention_mode=fa_pbrs llm_fd_pbrs_lambda=0.02 llm_fd_pbrs_alpha=0.0 llm_fd_pbrs_beta=0.0 llm_fd_pbrs_adaptive=False llm_fd_pbrs_random_features=True" ;;
    *) return 2 ;;
  esac
  printf '%s|%s\n' "$config" "$extra"
}
make_jobs(){
  ...
  for seed in $SEEDS; do for method in $METHODS; do printf 'abval|%s|%s|%s|%s|%s\n' "$ENV_KEY" "$TIME_LIMIT" "$T_MAX" "$method" "$seed" >> "$OUT_DIR/jobs_all.txt"; done; done
  if [[ "${SHUFFLE_JOBS:-1}" == "1" ]]; then python3 - "$OUT_DIR/jobs_all.txt" "${JOB_SHUFFLE_SEED:-19}" <<'PYSHUFFLE'
import random, sys
...
package_outputs(){ summarize_partial; local artifact="$ROOT_DIR/artifacts/AAAI_ab_validation_3seed_${STAMP}.tar.gz"; tar -C "$ROOT_DIR" -czf "$artifact" "results/$(basename "$OUT_DIR")" "run_ab_validation_3seed.sh"; log_status "ARTIFACT $artifact"; }
main(){
  log_status "A+B validation (3-seed) started at $(date) PARALLEL=$PARALLEL methods=$METHODS seeds=$SEEDS t_max=$T_MAX"
  ...
  wait "${pids[@]}"
  package_outputs
  log_status "A+B validation (3-seed) finished at $(date)"
}
main "$@"
